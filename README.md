...
- **상태 관리**: 모든 할 일 관련 상태를 중앙에서 관리
- **비즈니스 로직**: CRUD 작업 및 API 통신 처리
- **컴포넌트 조합**: 하위 컴포넌트들을 조합하여 전체 앱 구성

**주요 기능**:

- 할 일 데이터 상태 관리 (CRUD)
- API 통신 및 에러 처리
- 정렬 로직 및 드래그 앤 드롭 순서 관리
...
- **통합된 헤더**: 제목, 통계, 정렬 옵션을 한 곳에서 관리하여 일관성 유지
- **실시간 통계**: 진행률과 우선순위별 현황을 즉시 확인 가능
- **사용자 경험**: 헤더에서 모든 주요 정보를 한눈에 파악
...
- **드래그 앤 드롭**: 사용자가 직관적으로 순서 변경 가능
- **성능 최적화**: 가상화 없이도 효율적인 렌더링
- **사용자 정의**: 정렬 기준과 독립적인 개인화된 순서

**주요 기능**:

- 할 일 목록 표시 및 관리
- HTML5 Drag & Drop API를 활용한 순서 변경
- 로딩 상태 처리 및 빈 상태 메시지
...
- **인라인 편집**: 별도 모달 없이 빠른 수정 가능
- **상태 관리**: 완료 상태 토글 및 실시간 업데이트
- **사용자 경험**: 더블클릭으로 편집 모드 진입

**주요 기능**:

- 할 일 정보 표시 (제목, 설명, 우선순위, 완료 상태)
- 인라인 편집 시스템 (제목, 설명 수정)
- 완료 상태 토글 및 삭제 기능
...
- **모달 형태**: 배경과 분리된 집중된 입력 환경
- **유효성 검사**: 제목 필수 입력 등 데이터 검증
- **사용자 경험**: ESC 키나 배경 클릭으로 닫기 가능
...
- **통합 알림**: 에러, 성공, 연결 상태를 일관된 UI로 표시
- **자동 숨김**: 적절한 타이밍으로 자동 사라짐
